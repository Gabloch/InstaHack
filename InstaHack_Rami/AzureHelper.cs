...
namespace InstaHack_Rami
{
    public static class AzureHelper
    {
        public static async Task<UserInfo> GetOrCreateUserInfo(InstagramUser instagramUser)
        {
            ...
            try
            {
                output = (await App.UserInfoTable.Where(item => item.Username == instagramUser.Username).Take(1)
        ...
        public static async Task<Hashtag> GetOrCreateHashtag(Tag tag)
        {
            ...
            try
            {
                output = (await App.HashtagTable.Where(item => item.Name == tag.Name).Take(1).ToEnumerableAsync())
                    .FirstOrDefault();
        ...
        public static async Task<LinkHashtagUser> GetOrCreateLinkHashtagUser(Hashtag tag, UserInfo user)
        {
            ...
            try
            {
                output = (await App.LinkHashtagUserTable
                        .Where(item => item.UserId == user.Id && item.HashtagId == tag.Id).Take(1).ToEnumerableAsync())
                    .FirstOrDefault();
